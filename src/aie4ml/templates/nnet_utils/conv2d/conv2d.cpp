...
conv2d_base<ConfigT>::conv2d_base() {
  ...
  static_assert(!ConfigT::FILLS_BORDER || ConfigT::IN_ORIGIN_R == ConfigT::PAD_T,
                "a kernel-filled border starts exactly where this layer pads");
  ...
  static_assert(ConfigT::IN_ORIGIN_R + ConfigT::IN_H <= ConfigT::IN_ROWS &&
                    ConfigT::IN_ORIGIN_C + ConfigT::IN_W <= ConfigT::IN_COLS,
                "image fits its frame");
  static_assert(ConfigT::FILLS_BORDER || ConfigT::IN_H == ConfigT::IN_ROWS,
                "a delivered window is image all the way down; only its columns carry a border");
...
static inline void conv2d_zero_border(typename ConfigT::data_t* frame) {
  ...
  for (int cb = 0; cb < ConfigT::CB; ++cb) {
    ...
    for (int r = 0; r < ConfigT::IN_ROWS; ++r) {
      ...
      if constexpr (C1 % 2) {
        reinterpret_cast<int32*>(row + C1 * 8)[0] = 0;
        reinterpret_cast<int32*>(row + C1 * 8)[1] = 0;
      }
